...
\end{scope}
\node[btitle] at (b4.north west) {Frontend --- Next.js 16 · React 18};
...
\section{Relaciones geográficas entre celdas y zonas}
\label{sec:relaciones_geograficas}

Muchas consultas de SIPREH no se piden por celda, sino por \textbf{zona}: una cuenca, un municipio o un perímetro urbano. Para responderlas, el sistema necesita saber qué celdas caen dentro de cada zona y en qué proporción. Esa correspondencia es \emph{metadata geográfica} y, por eso, vive en la base de datos relacional, no en la capa de datos científicos.
...
\midrule
\texttt{grid\_cells}         & Las celdas del grid, identificadas por su fuente/resolución (ERA5, ERA5-Land, IMERG o CHIRPS) y su \texttt{cell\_id} (\texttt{lon\_lat}). \\
\texttt{spatial\_zones}      & Las entidades espaciales: cuenca, municipio o perímetro, cada una con su tipo, su identificador y su nombre. \\
\texttt{zone\_cell\_relations}& La unión entre ambas. Cada fila enlaza una zona con una celda y guarda el \textbf{área de intersección} (\texttt{area\_m2}) como peso de esa unión. \\
\bottomrule
\end{tabular}
\end{center}

\begin{definition}
\textbf{Agregación por zona:} el valor de una zona es el promedio de sus celdas ponderado por el área que cada celda aporta a la zona:
\[
\text{valor}_{\text{zona}} = \frac{\sum_c \text{valor}_c \times \text{área}_c}{\sum_c \text{área}_c}
\]
Como las áreas de intersección están precalculadas y almacenadas en \texttt{zone\_cell\_relations}, la agregación en tiempo de consulta se reduce a esta suma ponderada, sin recomputar geometría.
\end{definition}

\begin{remark}
Guardar las relaciones en tablas (en lugar de calcularlas al vuelo en cada petición) tiene dos ventajas: las consultas por zona son inmediatas y el cruce geométrico ---costoso--- se hace una sola vez. Añadir un nuevo conjunto de zonas se reduce a poblar \texttt{spatial\_zones} y \texttt{zone\_cell\_relations} mediante el sembrado correspondiente, de forma idempotente y sin tocar la interfaz.
\end{remark}
